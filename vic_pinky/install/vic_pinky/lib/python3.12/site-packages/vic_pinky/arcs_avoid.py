...
class FrontBackLeftRightBoxDetector(Node):
    def __init__(self):
        super().__init__('follow_avoid_node')
        self.get_logger().info("🚀 FollowAvoidNode 시작")
        ...
        self.pillar_positions = [
            (0.1,  0.18, 0.25),  # pillar1 x=1.0, y=0.5, radius=0.25m
            (-0.47,  0.18, 0.25),  # pillar2
            (-0.47, -0.18, 0.25),  # pillar3
            (0.1, -0.18, 0.25)   # pillar4
        ]
    ...
    def is_pillar_point(self, x, y):
        for cx, cy, radius in self.pillar_positions:
            dx = x - cx
            dy = y - cy
