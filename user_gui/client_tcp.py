...
from PyQt5.QtCore import QTimer, QThread, pyqtSignal, Qt, QPointF, QObject
...
class TcpCommunicator(QObject):
    ...
    def __init__(self, main_window=None, send_ip='192.168.0.145', receive_ip='0.0.0.0', send_port=12347, receive_port=23457):
        ...
        try:
            ...
            #바인딩 성공 후 로그 출력
            print(f"[TCP] 수신 대기 중 → {receive_ip}:{receive_port}")
        except Exception as e:
            print(f"[TCP] 수신용 소켓 설정 실패: {e}")
            self.receiver = None
    ...
    #tcp 패킷 전송 위해 메시지로 구성해서 서버에게 보내는 함수
    def send_tcp_message_um(self, command, payload_dict):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sender:
                sender.connect((self.send_ip, self.send_port))

                header = b'\x00' #1byte #0x00
                command_bytes = command.encode("utf-8")  #2bytes #ex) command = "KG" → command_bytes = b'KG' 
                #payload가 존재할 때만
                if payload_dict:
                    payload_json = json.dumps(payload_dict).encode("utf-8") #*bytes 
                                                                            #payload_dict = {"loadcell": 12345}
            ...
            print(f"[TCP 전송 오류](send_tcp_message_um): {e}")
        finally:
            sender.close()

    #클라이언트의 접속을 받아들이고 수신 스레드 시작.
    def _accept_connection(self):    
        try:
            #클라이언트의 연결 수락 (block 상태로 기다림)
            self.receiver_conn, addr = self.receiver.accept() #self.receiver_conn : 실제 통신용 소켓, addr: 연결된 클라이언트의 주소 정보
            #연결 성공 시 연결된 클라이언트 주소 출력
            print(f"[TCP] 수신 연결됨 from {addr}")
            #백그라운드에서 TCP 메시지를 계속 수신하는 스레드
            self.receiver_thread = threading.Thread(target=self._receive_tcp_data_mu)
            #_receive_tcp_data_mu() 백그라운드에서 실행되기 시작
            self.receiver_thread.start()
        except Exception as e:
            print(f"[TCP] 수신 연결 실패(_accept_connection): {e}")

    #연결된 소켓으로부터 실제 데이터 수신
    def _receive_tcp_data_mu(self):
        while self._running:
            print("수신 쓰레딩 러닝 중")
            try:
                ...
                print("[TCP 수신](_receive_tcp_data_mu): " + command)
                if command:
                    data = b''   # 비어 있는 바이트열로 초기화
                    ...
                    print(f" 수신한 json좀 살펴보자: {payload_dict}") 
                    #mu_2 수신
                    if command == "AR":
                        print("AR 들어감")
                ...
                print(f"[TCP 수신 오류](_receive_tcp_data_mu): {e}")
    ...
    def close(self):
        try:
            ...
            if self.receiver_conn:
                ...
                print("[TCP 닫는중] 수신 연결 소켓 닫힘")

            if self.receiver:
                self.receiver.shutdown(socket.SHUT_RDWR)
                self.receiver.close()
                print("[TCP 닫는중] 수신 서버 소켓 닫힘")
...
def speak(text):
    engine = pyttsx3.init()
    engine.setProperty('rate', 120)    #말하는 속도
    engine.setProperty('volume', 1.0)  # 불륨 ( 0.0 ~ 1.0)
    engine.say(text)
    engine.runAndWait()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        loadUi("/home/dong/project_local/final_project/user_gui/client.ui", self)
         # TCP 통신 객체 생성
        self.tcp = TcpCommunicator(main_window=self)
        ...
        self.place_coords ={             # 목적지 좌표 설정(임의값) - 맵 기반 좌표,pose 집어넣어야 함.
            ...
            'gate2': (2.0, 2.0, 0.0),
            'gate3': (3.0, 3.0, 0.0),
            'toilet_w': (4.0, 4.0, 0.0),
            'toilet_e': (5.0, 5.0, 0.0), 
            'shop_S': (6.0, 6.0, 0.0),
            'shop_L': (7.0, 7.0, 0.0),
            'shop_H': (8.0, 8.0, 0.0),
...
# =============================================================================
    ...
    def handle_mu3(self, payload_dict):
        if self.selected_function == 'leading_mode':
            ...
            self.tcp.send_tcp_message_um("ED", payload_dict)
            print("[TCP 전송] llm목적지 추가해서 ED 전송완료")

            llm_msg = payload_dict.get("llm_response") or ""
            print(f"[TCP 수신] llm목적지 : {llm_place} / 메시지: {llm_msg}")          
        else:
            llm_place = payload_dict.get("des_name")
            if llm_place:
...
# ===========================================================================
    ...
    def _go_loading(self):
        ...
        self.tcp.send_tcp_message_um("KG", payload_dict)
        print("[TCP] KG 전송 완료")
...
# ===========================================================================
    ...
    def _route_from_loading(self):
        if self.selected_function == 'following_mode':
            ...
            self.tcp.send_tcp_message_um("FW", payload_dict)
            print("[TCP] FW 전송 완료")
...
# ===========================================================================
    ...
    def _toggle_following_pause(self):
        ...
        if not self.is_paused_f:
            ...
            self.tcp.send_tcp_message_um("PS", payload_dict)
            print("[TCP] PS 전송 완료")
...
# ===========================================================================
    ...
    def _check_goal(self):
        ...
        self.tcp.send_tcp_message_um("CK", payload_dict)
        print("[TCP] CK 전송 완료")
...
# ===========================================================================
    ...
    def _start_guide(self):
        ...
        if not self.edit:  
            ...
            self.tcp.send_tcp_message_um("LD", payload_dict) # um_5 전송
            print("[TCP] LD 전송 완료")
        #목적지 편집상태면
        else: 
            payload_dict = {
            ...
            self.tcp.send_tcp_message_um("ED", payload_dict) # um_7 전송
            print("[TCP] ED 전송 완료")
            self.edit = False
...
# ===========================================================================
    ...
    def _toggle_guiding_pause(self):
        if not self.is_paused_g: #일시정지 눌렀을 때
            ...
            self.tcp.send_tcp_message_um("LD", payload_dict)
            print("[TCP] LD 전송 완료")
...
# ===========================================================================
    ...
    def _enter_decision(self):
        ...
        self.tcp.send_tcp_message_um("AR", payload_dict)
        print("[TCP] AR 전송 완료")
    ...
    def _wait_here(self):
        ...
        self.tcp.send_tcp_message_um("PS", payload_dict)  # um_4
        print("[TCP] PS 전송 완료")
        self.show_page("waiting")
...
# ===========================================================================
    ...
    def _send_um3_and_go_home(self):
        ...
        self.tcp.send_tcp_message_um("RT", payload_dict)
        print("[TCP] RT 전송 완료")
        self.show_page("main")
