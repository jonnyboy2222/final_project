...
CMD_SIZE = 2
# ---------- Protocol Utilities ----------
def encode_message(cmd: str, json_obj: dict) -> bytes:
    cmd_bytes = cmd.encode('utf-8')[:CMD_SIZE]
    json_bytes = json.dumps(json_obj).encode('utf-8')
    return HEADER_BYTE + cmd_bytes + json_bytes
def decode_stream(data: bytes):
    if len(data) < 3:
        return None
    if data[0:1] != HEADER_BYTE:
        print("[decode_stream] Invalid header")
        return None
    cmd = data[1:3].decode('utf-8')
    try:
        json_data = data[3:].decode('utf-8')
        payload = json.loads(json_data)
    except Exception as e:
        print(f"[decode_stream] JSON decode error: {e}")
        return None
    return cmd, payload
# ---------- Signal manager ----------
...
class ServerThread(threading.Thread):
    ...
    def run(self):
        login_data = {"id": self.user_id, "password": self.password}
